...
use utils::resized;
use utils::BLACK_RGBA;
use utils::WHITE_RGBA;
...
impl MirageTank {
    pub fn grey_output(
        ...
        let wimage = resized(&self.wimage, width, height).to_rgba8();
        let bimage = resized(&self.bimage, width, height).to_rgba8();
            ...
            .zip(oimage.pixels_mut())
        {
            ...
            let wc: f32 = f32::from(wpixel.to_luma()[0]) * wlight;
            let bc: f32 = f32::from(bpixel.to_luma()[0]) * blight;
    ...
    pub fn colorful_output(
            ...
            .zip(oimage.pixels_mut())
        {
            ...
            for i in 0..3 {
                wrgb[i] = f32::from(wpixel[i]) / 255.0 * wlight;
            }

            for i in 0..3 {
                brgb[i] = f32::from(bpixel[i]) / 255.0 * blight;
            }
